...
def populate_reviews():
    ...
    db.session.add(Review(title="John Hardy Women's Legends Naga Gold & Silver Dragon Station Chain Bracelet", type='jewelry', description="From our Legends Collection, the Naga was inspired by the mythical water dragon that protects the ocean's pearl. Wear facing inward to be bestowed with love and abundance, or outward for protection.", publishing_date='31-07-2021', price='75€', user_id=user.id))
    db.session.add(Review(title='Samsung 49-Inch CHG90 144Hz Curved Gaming Monitor (LC49HG90DMNXZA) – Super Ultrawide Screen QLED', description='49 INCH SUPER ULTRAWIDE 32:9 CURVED GAMING MONITOR with dual 27 inch screen side by side QUANTUM DOT (QLED) TECHNOLOGY, HDR support and factory calibration provides stunningly realistic and accurate color and contrast 144HZ HIGH REFRESH RATE and 1ms ultra fast response time work to eliminate motion blur, ghosting, and reduce input lag', publishing_date='22-10-2021', price='199€', user_id=user.id))
    db.session.add(Review(title='WD 4TB Gaming Drive Works with Playstation 4 Portable External Hard Drive', description="Expand your PS4 gaming experience, Play anywhere Fast and easy, setup Sleek design with high capacity, 3-year manufacturer's limited warranty", publishing_date='12-05-2021', price='150€', user_id=user.id))
    ...
    db.session.add(Review(title='Fjallraven - Foldsack No. 1 Backpack, Fits 15 Laptops', description='Your perfect pack for everyday use and walks in the forest. Stash your laptop (up to 15 inches) in the padded sleeve, your everyday', publishing_date='01-02-2023', price='59€', user_id=user.id))
    db.session.commit()
